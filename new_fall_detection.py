        ...
        for height in heights:
            for track in tracks:
                if int(track[0]) == int(height[0]):
                    ...
                    valid_heights = [h for h in self.heightBuffer[tid] if h != -5]
                    if len(valid_heights) > 0:
                        reference_height = np.median(valid_heights)
                        # Adjust threshold based on height variability
                        height_std = np.std(valid_heights) if len(valid_heights) > 1 else 0
                        adaptive_threshold = self.fallingThresholdProportion * (1 + height_std / reference_height)
                        if self.heightBuffer[tid][0] < adaptive_threshold * reference_height:
                            self.fallFrameCounter[tid] += 1
                            if self.fallFrameCounter[tid] >= self.minFallFrames:
                                self.fallBufferDisplay[tid] = self.numFramesToDisplayFall
                        else:
                            self.fallFrameCounter[tid] = 0
